    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                title={lastSyncTime ? `Última sincronização: ${lastSyncTime.toLocaleTimeString()}` : 'Pronto para usar nuvem'}
            >
                {getIcon()}
                <span>{getLabel()}</span>
            ...
            {dropdownOpen && state !== 'syncing' && state !== 'offline' && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-slate-800 border border-slate-700 rounded-lg shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                    <div className="p-2 border-b border-slate-700/50 flex flex-col gap-1">
                        <span className="text-xs font-semibold text-slate-400 px-2 py-1 uppercase tracking-wider">Ações Manuais</span>
                    </div>
                    ...
                    >
                        ...
                        <div>
                            <span className="block text-sm font-medium">Baixar do GitHub</span>
                            <span className="block text-xs text-slate-500 mt-0.5">Sobrescreve alterações locais com a Nuvem</span>
                        </div>
                    </button>
                    ...
                    >
                        ...
                        <div>
                            ...
                            <span className="block text-xs text-slate-500 mt-0.5">Envia o local exato ignorando a nuvem</span>
                        </div>
                    </button>
                </div>
            )}
