...
# <comando de build do AGENTS.md, ex: pnpm build>
# <comando de testes do AGENTS.md, ex: pnpm test>
...
# Se Rust
# cargo fmt --check   ← obrigatório: formata o código antes de commitar
# cargo clippy
# cargo test
```

Capture o output completo de cada comando. Para Rust, execute **sempre** as três ferramentas na ordem acima: `fmt --check` detecta divergências de estilo antes do clippy.
...
- `any` explícito → **AVISO**
- `invoke()` chamado diretamente em componentes ou hooks, fora de `lib/api/` → **ERRO** (viola camada de API declarada no AGENTS.md)
...
- `console.log` / `console.error` em código de produção (use toast ou logger do projeto) → **AVISO**
...
- `cargo fmt --check` reporta diffs (código não formatado) → **AVISO**
- Funções `#[tauri::command]` sem `#[tracing::instrument]` (se AGENTS.md exige) → **AVISO**
- Structs com `fn new() -> Self` sem `impl Default` → **AVISO** (clippy `new_without_default`)
- `&PathBuf` em assinatura de função onde `&Path` serve → **AVISO** (clippy `ptr_arg`)
